...
@Client.on_message(
    commandpro(["/play", "00" ,"شغل" ,"تشغيل" ,"سافو"])
    & filters.group
    & ~filters.edited
    & ~filters.forwarded
    & ~filters.via_bot
...
async def play(_, message: Message):
    ...
    lel = await message.reply("**🔄 جـاري البـحث...**")
    ...
    if audio:
        ...
        if len(message.command) < 2:
            return await lel.edit(
                "**قـم باعطائي اسم الموسيقي \n او الرد علي ملف صوتي لتشغيـله...**"
            )
        await lel.edit("**🔎جـاري البحـث...**")
        query = message.text.split(None, 1)[1]
        # print(query)
        await lel.edit("**🔄 يتـم التحمـيل...**")
        try:
    ...
    if int(message.chat.id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            caption="**⚡اهلا عزيزي⚡ \n تم اضافه الموسيقي \n لقـائمه الانتظار  » `{}` 🎶 ...**".format(position),
...
@Client.on_message(commandpro(["/skip", "/next", "اللي بعدو", "تخطي"]) & other_filters)
@errors
@authorized_users_only
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALL:
        await message.reply_text("**⚡اهلا عـزيزي⚡ \n يتم التشغيـل الان ...**")
    else:
...
@Client.on_message(commandpro(["/end", "End", "/stop", "اسكت"]) & other_filters)
@errors
@authorized_users_only
